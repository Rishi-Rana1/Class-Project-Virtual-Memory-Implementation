...
int8_t ownerGlobal;             //global var to keep track of owner data
...
void kernel(const char* command) {
    ...
    console_clear();
    timer_init(HZ);
    log_printf("dbcibcohewbcoefwbvhbefwovbefwvbwe\n");
    ...
    //00100000 in hex / 2^12 (page size)
    if (virtual_memory_map(kernel_pagetable, 0, 0, PROC_START_ADDR, PTE_P|PTE_W, NULL) == -1) {
        log_printf("Error, could not allocate entire kernel");
    }
    if (virtual_memory_map(kernel_pagetable, (uintptr_t) console, (uintptr_t) console, PAGESIZE, PTE_P|PTE_U|PTE_W, NULL) == -1) {
        log_printf("Error, could not allocate console correctly");
    }
...
x86_64_pagetable* vmmAlloc() {
    //loops through page array to find free page, then assigns and returns it if found, else returns NULL
    x86_64_pagetable* new_page = NULL;
    for (uintptr_t i = 0; i < (MEMSIZE_PHYSICAL); i += PAGESIZE) {
        if (pageinfo[PAGENUMBER(i)].refcount == 0) {
            new_page = (x86_64_pagetable*)i;
            break;
        }
    }
    assign_physical_page((uintptr_t)new_page, ownerGlobal);
    memset(new_page, 0, PAGESIZE);
    return new_page;
}

x86_64_pagetable* copy_pagetable(x86_64_pagetable* pagetable, int8_t owner) {
    //finds an empty page and goes through all of virtual memory to map pages according to their permissions
    ownerGlobal = owner;
    x86_64_pagetable* new_pagetable = vmmAlloc();
    for (uintptr_t k = 0; k < MEMSIZE_VIRTUAL; k += PAGESIZE) {
        vamapping new = virtual_memory_lookup(pagetable, k);
        if (new.pn != -1) {
            if(virtual_memory_map(new_pagetable, k, new.pa, PAGESIZE, new.perm, &vmmAlloc) < 0){
                return NULL;
            };
        }
    }
...
void process_setup(pid_t pid, int program_number) {
    // copy page table, assign process' page table to the copy
    ...
    //part 4; Allow each process to use full virtual memory space and allocate and map a physical page found.
    processes[pid].p_registers.reg_rsp = MEMSIZE_VIRTUAL;
    ownerGlobal = pid;
    uintptr_t pa = (uintptr_t) vmmAlloc();
    uintptr_t stack_page = processes[pid].p_registers.reg_rsp - PAGESIZE;
    // assign_physical_page(stack_page, pid);
    virtual_memory_map(processes[pid].p_pagetable, stack_page, pa,
                       PAGESIZE, PTE_P | PTE_W | PTE_U, vmmAlloc);                   
...
void exception(x86_64_registers* reg) {
    ...
    case INT_SYS_FORK:
        ...
        for (i = 1; i < NPROC; i++) {
            if ((processes[i]).p_state == P_FREE) {
                ...
                x86_64_pagetable *copy = copy_pagetable(current->p_pagetable, i);
                processes[i].p_pagetable = copy;
                break;
                //copies current processes and kernel and afterwards make sure to lookup each page and map each page with perms return child process ID, set rax to that
            }
        }
        ...
        for (uintptr_t k = PROC_START_ADDR; k < MEMSIZE_VIRTUAL; k += PAGESIZE){
            //loop through virtual processes space, find physical page, and map virtual process space to physical space.
            vamapping new = virtual_memory_lookup(processes[i].p_pagetable, k);
            if (new.pn != -1 && (new.perm & PTE_W)) {
                ownerGlobal = i;
                uintptr_t page = (uintptr_t) vmmAlloc();
                memcpy((void*)page, (void*)new.pa, PAGESIZE);
                virtual_memory_map(processes[i].p_pagetable, k, page, PAGESIZE, new.perm, vmmAlloc);
            }
        }
    ...
    case INT_SYS_PAGE_ALLOC: {
        ...
        if (addr < PROC_START_ADDR) {
            //return -1 if address being allocated in in kernel space
            current->p_registers.reg_rax = -1;
            break;
        }
        else {
            //else find a physical page (if a page is found) and map it with virtual address
            ownerGlobal = current->p_pid;
            x86_64_pagetable *page = vmmAlloc();
            if (page == NULL){
                current->p_registers.reg_rax = -1;
                break;
            }
            //int r = assign_physical_page(addr, current->p_pid);
            virtual_memory_map(current->p_pagetable, addr, (uintptr_t)page,
